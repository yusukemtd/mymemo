...
| `menu <id>` | ネイティブメニュー相当。`new` `open` `save` `save_as` `revert` `close_tab` `find` `grep` `goto_line` `toggle_whitespace` `toggle_wrap` `toggle_fold_gutter` `toggle_close_brackets` `toggle_word_completion` `fold_code` / `unfold_code` / `fold_all` / `unfold_all` `zoom_in` `zoom_out` `zoom_reset` `quit` `theme:light` `theme:solarized-dark` `open_enc:CP932` `recent:0`(最近使ったファイルの 0 番目) `recent_clear` `tabsize:2`(タブ幅) `soft_tabs`(ソフトタブ切替) `transform:sort_asc` / `sort_desc` / `unique` / `remove_blank`(テキスト変換) `set_enc:CP932`(保存時の文字コード変更) `convert_eol:CRLF` |
