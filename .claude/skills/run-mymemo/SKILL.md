...
| `menu <id>` | ネイティブメニュー相当。`new` `open` `save` `save_as` `close_tab` `find` `grep` `goto_line` `toggle_whitespace` `toggle_wrap` `zoom_in` `zoom_out` `zoom_reset` `quit` `theme:light` `theme:solarized-dark` `open_enc:CP932` |
