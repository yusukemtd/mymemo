...
| `menu <id>` | ネイティブメニュー相当。`new` `open` `save` `save_as` `revert` `close_tab` `find` `grep` `goto_line` `toggle_whitespace` `toggle_wrap` `zoom_in` `zoom_out` `zoom_reset` `quit` `theme:light` `theme:solarized-dark` `open_enc:CP932` `recent:0`(最近使ったファイルの 0 番目) `recent_clear` `tabsize:2`(タブ幅) `soft_tabs`(ソフトタブ切替) |
